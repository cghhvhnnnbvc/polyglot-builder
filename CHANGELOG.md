...
- **fix: 默认窗口下选项区裁切, "硬件编码"看不到**: v1.2 把 4 组控件排在单行, 而 `pack(side=LEFT)` 不换行, 高 DPI/大字体下排在最后的"硬件编码"被裁在窗口右缘 (100% 缩放下刚好够, 故低缩放下不复现)。改为**按功能分组的两行**: 第一行 Deflate (内层 RAR 压缩), 第二行"压缩表面视频"整组 (启用开关 + 档位下拉 + 硬件编码) 同排并左缩进, 语义与防裁切兼得; 实测 100%~220% 等效缩放下两行均不超宽
...
- 新增 6 个回归测试: 压缩三控件同排分组守护、150% 缩放行宽守护、档位反向映射守护、下拉默认文案守护、未勾选不压缩守护; 测试 145 → 151
