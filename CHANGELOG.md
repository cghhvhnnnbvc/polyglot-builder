...
## v1.2.2 (2026-09-04) — 台账恢复安全修复

- **版本号**: 统一为 1.2.2 (`VERSION` 常量 / bat 标题与 2 处横幅 / GUI 文件头 / README 徽章)
- **fix: 恢复台账备份时可能清空数据**: 「资源台账」创建流程此前对选中的路径无条件清空写入; 现选到已存在的 `.json` 时直接使用、绝不覆盖 (新增 2 个回归测试)
- `docs/ledger.md` 新增"备份与恢复"一节: JSON 是唯一数据源 (备份只需复制它), HTML 查看页为衍生物可不备, `ledger_config.json` 不用备; 旧版单文件 HTML 台账会自动迁移

## v1.2.1 (2026-09-04) — 界面布局修复

- **版本号**: 统一为 1.2.1 (`VERSION` 常量 / bat 标题与 2 处横幅 / GUI 文件头 / README 徽章); 版本格式测试放开为允许三段语义化版本
- **fix: 默认窗口下选项区裁切, "硬件编码"看不到**: v1.2 把 4 组控件排在单行, 而 `pack(side=LEFT)` 不换行, 高 DPI/大字体下排在最后的"硬件编码"被裁在窗口右缘 (100% 缩放下刚好够, 故低缩放下不复现)。改为**按功能分组的两行**: 第一行 Deflate (内层 RAR 压缩), 第二行"压缩表面视频"整组 (启用开关 + 档位下拉 + 硬件编码) 同排, 两行左边缘对齐不缩进; 实测 100%~220% 等效缩放下两行均不超宽
- **档位下拉文案缩短** (如 "中 1.5M/720p 推荐"), 宽度 26→18 给整组同排留余量; 档位 key 改为反向映射解析 (不再靠前缀匹配), 新增对应回归测试
- **复选框与角标字号改为跟随 `FONT_ENTRY` 派生** (默认 10→11 / 8, 视觉不变), 使高 DPI 下的布局计算与真实渲染一致
- **fix: "压缩表面视频" 复选框此前不生效**: `_selected_quality` 只解析下拉文案, 未勾选时仍返回档位 key, 导致 `_run` 照旧压缩外层视频 (复选框形同虚设); 现以复选框为准, 未勾选返回 None
- **fix: 档位默认值错误**: 下拉框初始化时被 `set(quality_labels[0])` 覆盖为第一项"高", 与"中 (推荐)"矛盾; 现默认显示 medium 档文案
- **fix: 恢复台账备份时可能清空数据**: 「资源台账」创建流程此前对选中的路径无条件清空写入; 现选到已存在的 `.json` 时直接使用、绝不覆盖 (新增 2 个回归测试); `docs/ledger.md` 新增"备份与恢复"一节 (JSON 是唯一数据源, HTML 为衍生物可不备)
- 新增 8 个回归测试: 压缩三控件同排分组、150% 缩放行宽、档位反向映射、下拉默认文案、未勾选不压缩、台账恢复不覆盖、新位置创建等守护; 测试 145 → 153
